...
export function Sidebar({ userName = "Usuário", userEmail }: SidebarProps) {
  ...
  return (
    <aside className="w-[220px] min-h-screen sidebar border-r border-border-subtle flex flex-col flex-shrink-0">
      {/* Logo */}
      <div className="flex items-center px-4 py-5">
        ...
        <div>
          ...
          <p className="text-[9px] uppercase tracking-[2px] text-subtle">
            Pessoal
          </p>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 pb-4 overflow-y-auto">
        {navSections.map((section) => (
          <div key={section.title} className="mb-3">
            <p className="text-[9px] uppercase tracking-[2px] font-semibold text-white border border rounded-md p-1.5 px-2 mb-1.5 mt-3">
              {section.title}
            </p>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] mb-0.5 transition-colors ${
                    isActive
                      ? "bg-blue-900 text-text font-semibold border-l-2 border-accent pl-2"
      ...
      <div className="border-t border-border-subtle p-3">
        <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-card-hover transition-colors">
          ...
          <Link
            href="/configuracoes"
            className="w-6 h-6 rounded-md bg-card flex items-center justify-center text-muted hover:text-accent transition-colors flex-shrink-0"
          >
            <Settings size={12} />
          </Link>
        </div>
